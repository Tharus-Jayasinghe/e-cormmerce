...
{
    // Fetch the cart item and its related product
    $cart = Cart::with('product')->findOrFail($id);

    // Get the current user's cart count
    $count = Auth::id() ? Cart::where('user_id', Auth::id())->count() : 0;

    // Return the order form view with the cart details
    return view('home.order_product', compact('cart', 'count'));
}
...
public function viewProducts() {
    ...
    return view('admin.view_products', compact('products', 'categories'));
}
 
public function Category()
{
    $data = Category::all(); // Fetch all categories
    return view('home.category', compact('data')); // Pass categories to view
}
